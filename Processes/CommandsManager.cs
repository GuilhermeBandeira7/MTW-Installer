...
namespace InstallerMTW.Processes {
    ...
    public class CommandsManager {
        ...
        private void CreateUnique() {
            ...
            Equipment? selected = DbManager._context.Equipments.Include(x => x.RtspConfigs).Where(equip => equip.Id == selectedID).FirstOrDefault();

            if (selected != null && selected.Id > 0) {
                Record? record = DbManager._context.Records.Find(selected.Id);
                if (record != null) {
                    Directory.CreateDirectory($"{record.Path}/{record.Id}_Record");
                    string recordString = $"ffmpeg -max_delay 5000000 -rtsp_transport tcp -allowed_media_types video -i \"{selected.RtspConfigs.First().OutputRtsp}\" -vcodec " +
                             $"copy -map 0 -f segment -segment_time 20 -strftime 1 {record.Path}/{record.Id}_Record/%Y%m%d%H%M%S.mkv";
        ...
        private void CreateRangeOfRecordService() {
            if (SelectedRange.Count > 0) {
                for (int cont = 0; cont <= SelectedRange.Count; cont++) {
                    Record? record = DbManager._context.Records.Include(x => x.RtspConfigs).Where(x => SelectedRange[cont].Id == x.Id).FirstOrDefault();
                    if (record != null) {
                        Directory.CreateDirectory($"{record.Path}/{record.Id}_Record");
                        string recordString = $"ffmpeg -max_delay 5000000 -rtsp_transport tcp -allowed_media_types video -i \"{record.RtspConfigs.First().OutputRtsp}\" -vcodec" +
                                 $" copy -map 0 -f segment -segment_time 20 -strftime 1 {record.Path}/{record.Id}_Record/%Y%m%d%H%M%S.mkv";
                ...
                throw new ProcessException("The range of selected RTSP is empty.");
            }
        }
        ...
        private void CreateDirectory(string path, string directoryName) {
            if (!Directory.Exists(path + "/" + directoryName)) {
                ExecuteCmd($"mkdir {path}/{directoryName}");
            }
        }
