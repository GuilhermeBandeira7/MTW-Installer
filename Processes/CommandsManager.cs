...
namespace InstallerMTW.Processes {
    ...
    public class CommandsManager {
        ...
        public void ListAllRtsp() {
            IEnumerable<string> rtspList = new List<string>();
            rtspList = DbManager.GetPrimaryRtsp();
            foreach (string rtsp in rtspList) {
                Console.WriteLine(rtsp);
            }

            SelectRange(rtspList.Count());
        }

        public void SelectRange(int contElements) {
            Console.WriteLine("Select the range of id's to operate: ");
            string range = Console.ReadLine().ToString();
        }
